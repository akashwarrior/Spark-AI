...
export default function BaseChat({ className }: { className: string }) {
  ...
  return (
    <div className={`flex flex-col h-full ${className}`}>
      <div className="flex-1 overflow-auto p-4 space-y-4">
        {messageStore.map(({ role, steps, createdAt }, id) => (
          <motion.div
            ...
            className={cn(
              ...
              role === "assistant"
                ? "bg-secondary/30 backdrop-blur-xs border border-border/50"
                : "bg-primary/5 backdrop-blur-xs",
            )}
          >
            <div className="w-8 h-8 rounded-lg border border-border/50 flex items-center justify-center backdrop-blur-xs shrink-0">
                    ...
                    <AnimatePresence
                      ...
                      {type !== undefined ?
                        <motion.div
                          ...
                          animate={{ opacity: 1, y: 0 }}
                          exit={{ opacity: 0, y: -20 }}
                          className="flex items-center gap-2 p-2 rounded-lg bg-secondary/30 backdrop-blur-xs border border-border/50 mb-4"
                        >
        ...
        {status === "submitted" && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -20 }}
            className="flex gap-3 p-4 rounded-lg bg-secondary/30 backdrop-blur-xs border border-border/50"
          >
            <div className="w-8 h-8 rounded-lg border border-border/50 flex items-center justify-center backdrop-blur-xs shrink-0">
      ...
      <form onSubmit={submitInput} className="p-4 border-t border-border backdrop-blur-xs">
        <div className="flex gap-2">
          <Textarea
            value={input}
            onChange={handleInputChange}
            placeholder="Describe the component or functionality you want to add..."
            className="min-h-[2.5rem] max-h-32 bg-secondary/50 backdrop-blur-xs border-secondary-foreground/10 transition-colors focus:bg-secondary/80 resize-none"
            onKeyDown={(e) => {
